        ...
        {quizCategories.map((category) => {
          ...
          return (
            <Button
              key={category.id}
              variant="outline"
              className={`justify-start border-cyber-blue/30 hover:bg-cyber-blue/20 hover:border-cyber-blue ${!quizExists ? 'opacity-50' : ''}`}
        ...
        {advancedCategories.map((category) => {
          ...
          return (
            <Button
              key={category.id}
              variant="outline" 
              className={`justify-start border-cyber-pink/30 hover:bg-cyber-pink/20 hover:border-cyber-pink ${!quizExists ? 'opacity-50' : ''}`}
              onClick={() => onSelectQuiz(category.id)}
              disabled={!quizExists}
            >
